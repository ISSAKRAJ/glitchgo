...
export default function Dashboard() {
  ...
  // Filter AI Complaints vs Actual Leads
  const actualLeads = leads.filter(l => l.deadline !== 'complaint');
  const complaints = leads.filter(l => l.deadline === 'complaint');
      ...
      <main className="max-w-7xl mx-auto p-6 py-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="glass px-6 py-4 rounded-xl flex flex-col items-center">
            <span className="text-3xl font-bold text-brand-blue">{actualLeads.length}</span>
        ...
        {complaints.length > 0 && (
          <div className="mt-12">
            <div className="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3">
              <div>
                <h2 className="text-2xl font-bold text-red-400 mb-1 flex items-center gap-2">
                  <AlertCircle size={24} /> Escalated Complaints
                </h2>
                <p className="text-gray-400 text-sm">Tickets secretly auto-generated by the GlitchGo AI Chatbot when a user complains.</p>
              </div>
              <div className="glass px-4 py-2 rounded-lg flex flex-shrink-0 items-center gap-2 border border-red-500/20 max-w-fit">
                <span className="text-xl font-bold text-red-400">{complaints.length}</span>
                <span className="text-xs text-red-400/70 tracking-wider">TICKETS</span>
              </div>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {complaints.map(ticket => (
                <div key={ticket.id} className="bg-dark-card border border-red-500/20 rounded-xl p-6 relative overflow-hidden group transition-all hover:border-red-500/40 hover:-translate-y-1">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-red-500/5 rounded-full blur-3xl -mr-16 -mt-16 transition-all group-hover:bg-red-500/10"></div>
                  
                  <div className="flex justify-between items-start mb-4 relative z-10">
                    <span className="text-xs text-gray-500 font-mono">{new Date(ticket.created_at).toLocaleString()}</span>
                    <span className="bg-red-500/10 text-red-400 px-2 py-0.5 rounded text-[10px] font-bold tracking-wider border border-red-500/20">URGENT</span>
                  </div>
                  
                  <p className="text-white text-sm mb-4 leading-relaxed font-medium relative z-10">"{ticket.problem}"</p>
                  
                  <div className="flex items-center gap-2 text-brand-orange text-sm font-mono bg-white/5 p-2 rounded-lg border border-white/5 relative z-10">
                    <Terminal size={14} className="text-gray-500 flex-shrink-0" />
                    <span className="line-clamp-1">{ticket.contact}</span>
                  </div>
                </div>
              ))}
            </div>
